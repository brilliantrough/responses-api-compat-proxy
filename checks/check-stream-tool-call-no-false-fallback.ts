...
async function main() {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'responses-proxy-stream-tool-call-'));
  ...
  const primary = createServer(async (req, res) => {
    if (req.method === 'POST' && req.url === '/v1/responses') {
      ...
      res.end([
        ...
        'data: {"type":"response.created","response":{"id":"resp_tool","status":"in_progress","model":"tool-model"}}',
        '',
        ...
        'data: {"type":"response.completed","response":{"id":"resp_tool","status":"completed","model":"tool-model","usage":{"input_tokens":10,"output_tokens":8,"total_tokens":18},"output":[{"type":"function_call","name":"get_weather","call_id":"call_1","arguments":"{\\"city\\":\\"SF\\"}"}]}}',
        '',
      ].join('\n'));
      return;
    }
    ...
    if (req.method === 'GET' && req.url === '/v1/models') {
      ...
      res.end(JSON.stringify({ object: 'list', data: [{ id: 'tool-model', object: 'model' }] }));
      return;
    }
  ...
  try {
    ...
    assert.equal(fallbackRequests, 0, 'fallback must NOT be reached for a valid tool-call-only stream');
    ...
    console.log('Stream tool-call no-false-fallback check passed.');
  } finally {
    proxy.kill('SIGTERM');
    await Promise.race([
      once(proxy, 'exit'),
