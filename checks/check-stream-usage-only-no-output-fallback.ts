...
async function main() {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), 'responses-proxy-stream-usage-only-'));
  ...
  const primary = createServer(async (req, res) => {
    if (req.method === 'POST' && req.url === '/v1/responses') {
      ...
      res.end([
        ...
        'data: {"type":"response.created","response":{"id":"resp_usage_only","status":"in_progress","model":"broken-model"}}',
        '',
        'event: response.completed',
        'data: {"type":"response.completed","response":{"id":"resp_usage_only","status":"completed","model":"broken-model","usage":{"input_tokens":14903,"output_tokens":64,"total_tokens":14967},"output":[]}}',
        '',
      ].join('\n'));
      return;
    }
  ...
  const fallback = createServer(async (req, res) => {
    if (req.method === 'POST' && req.url === '/v1/responses') {
      ...
      res.end([
        ...
        '',
        'event: response.completed',
        'data: {"type":"response.completed","response":{"id":"resp_good","status":"completed","model":"fallback-model","usage":{"input_tokens":1,"output_tokens":2,"total_tokens":3},"output":[{"type":"message","role":"assistant","status":"completed","content":[{"type":"output_text","text":"real answer","annotations":[]}]}]}}',
        '',
      ].join('\n'));
      return;
    }
  ...
  try {
    ...
    console.log('Stream usage-only no-output fallback check passed.');
  } finally {
    proxy.kill('SIGTERM');
    await Promise.race([
      once(proxy, 'exit'),
